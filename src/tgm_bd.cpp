...
extern "C"
{
					   ...
					   double D[], double data[], int *n, double *nu, double mu[], double tu[])
	{
		...
		for (int i_mcmc = 0; i_mcmc < iteration; i_mcmc++)
		{
			...
			{
				++print_counter;
				((i_mcmc + 1) != iteration) ? Rprintf("%i%%->", print_c * print_counter) : Rprintf(" done");
			}
			...
			if (i_mcmc >= burn_in)
			{
				...
				F77_NAME(daxpy)
				(&pxp, &weight_C, &K[0], &one, &K_hat_Cpp[0], &one);
						...
						double data[], int *n, double *nu, double mu[], double tu[])
	{
		...
		for (int i_mcmc = 0; i_mcmc < iteration; i_mcmc++)
		{
			...
			if (i_mcmc >= burn_in)
			{
				...
				F77_NAME(daxpy)
				(&pxp, &weight_C, &K[0], &one, &K_hat[0], &one);
