...
void rwish_c(double Ts[], double K[], int *b, int *p)
{
	...
	F77_NAME(dtrmm)
	(&side, &upper, &transN, &transN, &dim, &dim, &alpha, Ts, &dim, &psi[0], &dim FCONE FCONE FCONE FCONE);

	// K = t(C) %*% C
	// LAPACK function to compute  C := alpha * A * B + beta_zero * C
	F77_NAME(dgemm)
	(&transT, &transN, &dim, &dim, &dim, &alpha, &psi[0], &dim, &psi[0], &dim, &beta_zero, K, &dim FCONE FCONE);
}
...
void rgwish_c(int G[], double Ts[], double K[], int *b, int *p, double *threshold)
{
	...
	vector<double> sigma(sigma_start); // Should be IN/OUT
	...
	while (mean_diff > threshold_c)
	{
		...
		for (i = 0; i < dim; i++)
		{
			...
			if (size_node > 0)
			{
				...
				F77_NAME(dgemm)
				(&transN, &transN, &dim, &one, &dim, &alpha, &sigma[0], &dim, &beta_star[0], &dim, &beta_zero, &sigma_start_i[0], &dim FCONE FCONE);
				  ...
				  vector<double> &sigma_start_N_i, vector<double> &sigma_N_i, vector<int> &N_i)
{
	...
	char transT = 'T', transN = 'N', side = 'R', upper = 'U', diagN = 'N';
...
#pragma omp parallel for
	...
	F77_NAME(dtrmm)
	(&side, &upper, &transN, &diagN, &dim, &dim, &alpha, Ts, &dim, &sigma_start[0], &dim FCONE FCONE FCONE FCONE);
...
#pragma omp parallel for
	...
	F77_NAME(dtrsm)
	(&side, &upper, &transN, &diagN, &dim, &dim, &alpha, &sigma_start[0], &dim, &inv_C[0], &dim FCONE FCONE FCONE FCONE);

	// sigma_start = inv_C %*% t( inv_C )
	F77_NAME(dgemm)
	(&transN, &transT, &dim, &dim, &dim, &alpha, &inv_C[0], &dim, &inv_C[0], &dim, &beta_zero, &sigma_start[0], &dim FCONE FCONE);
	...
	while ((mean_diff > threshold_c) and (counter < 5000))
	{
		...
		for (i = 0; i < dim; i++)
		{
			...
			if (size_node_i > 0)
			{
				...
				// A * X = B   for   sigma_start_N_i := (sigma_N_i)^{-1} * sigma_start_N_i
				F77_NAME(dposv)
				(&upper, &size_node_i, &one, &sigma_N_i[0], &size_node_i, &sigma_start_N_i[0], &size_node_i, &info FCONE);
				...
				F77_NAME(dsymv)
				(&upper, &dim, &alpha, sigma, &dim, &beta_star[0], &one, &beta_zero, &sigma_i[0], &one FCONE);
...
void log_exp_mc(int G[], int nu[], int *b, double H[], int *check_H, int *mc, int *p, double f_T[])
{
	...
	if (*check_H == 1)
	{
		for (iter = 0; iter < mc_iter; iter++)
		{
				...
				for (j = i + 1; j < dim; j++)
				{
					...
					if (G[ij] == 0)
					{
						...
						if (i > 0)
						{
							...
							{
								if (sumPsi == R_PosInf)
									sumPsi = max_numeric_limits_ld;
								if (sumPsi == R_NegInf)
									sumPsi = min_numeric_limits_ld;
						...
						if (psi[ij] == R_PosInf)
							psi[ij] = max_numeric_limits_ld;
						if (psi[ij] == R_NegInf)
							psi[ij] = min_numeric_limits_ld;
			...
			// checking Inf values
			if (f_T[iter] == R_PosInf)
				f_T[iter] = max_numeric_limits_ld;
		}
	}
	else
	{
			...
			for (i = 0; i < dim - 1; i++)
				for (j = i + 1; j < dim; j++)
				{
					...
					if (G[ij] == 0)
					{
						...
						{
							if (sumPsiH == R_PosInf)
								sumPsiH = max_numeric_limits_ld;
							if (sumPsiH == R_NegInf)
								sumPsiH = min_numeric_limits_ld;
						...
						if (i > 0)					// if( i > 1 )
							for (r = 0; r < i; r++) // for( r in 1 : ( i - 1 ) )
							{
								...
								for (h = r; h < i + 1; h++)
								{
									if (sumPsiHi == R_PosInf)
										sumPsiHi = max_numeric_limits_ld;
									if (sumPsiHi == R_NegInf)
										sumPsiHi = min_numeric_limits_ld;
