				...
				if ((g_prior[ij] != 0.0) or (g_prior[ij] != 1.0))
				{
					if (G[ij])
					{
						...
						if (size_node_i_new > 0)
						{
							...
							count_mb = 0;
							for (t = 0; t < dim; t++)
								if (G[nodexdim + t] and t != j)
									mb_node_i_new[count_mb++] = t;
						}
						...
						if (size_node_j_new > 0)
						{
							...
							count_mb = 0;
							for (t = 0; t < dim; t++)
								if (G[nodexdim + t] and t != i)
									mb_node_j_new[count_mb++] = t;
						}
					}
					else
					{
						...
						count_mb = 0;
						for (t = 0; t < dim; t++)
							if (G[nodexdim + t] or t == j)
								mb_node_i_new[count_mb++] = t;

						nodexdim = j * dim;
						count_mb = 0;
						for (t = 0; t < dim; t++)
							if (G[nodexdim + t] or t == i)
								mb_node_j_new[count_mb++] = t;
					}
					 ...
					 int max_range_nodes[], double *alpha_ijl, int *n)
	{
		...
		{ // n_ijl
			...
			for (j = 0; j < (max_range_node_j + 1); j++)
			{
				...
				fam_conf_count[j] = 0;
				for (i = 0; i < *length_freq_data; i++)
					if (data[node_x_lf + i] == j)
						fam_conf_count[j] += freq_data[i];
			}
		...
		if (*size_node > 0)
		{
			*log_mpl_node = 0.0;
			for (l = 0; l < size_mb_conf; l++) // collects the necessary statistics from the data and calculates the score
			{
				...
				mb_conf_count = 0;
				for (i = 0; i < *length_freq_data; i++)
					if (data_mb[i] == mb_conf_l)
						mb_conf_count += freq_data[i];
				...
				for (j = 0; j < (max_range_node_j + 1); j++)
				{
					fam_conf_count[j] = 0;
					for (i = 0; i < *length_freq_data; i++)
						if ((data[node_x_lf + i] == j) && (data_mb[i] == mb_conf_l))
							fam_conf_count[j] += freq_data[i];
				}
